...
from .utils import center_scene, random_rotation, add_noise
...
class Trainer(object):
    ...
    def train(self, scenes, goals, epoch):
        ...
        self.log.info({
            'type': 'train-epoch',
            'epoch': epoch + 1,
            'loss': round(self.batch_size * epoch_loss / (len(scenes)), 5),
    ...
    def val(self, scenes, goals, epoch):
        ...
        self.log.info({
            'type': 'val-epoch',
            'epoch': epoch + 1,
            'loss': round(self.batch_size * val_loss / (len(scenes)), 3),
    ...
    def train_batch(self, batch_scene, batch_scene_goal, batch_split):
        ...
        # outputs = outputs[-self.pred_length:-1]

        primary_indicies = batch_split[:-1]
        outputs = outputs[-self.pred_length + 1:, primary_indicies]
        prediction_truth = prediction_truth[:, primary_indicies]
        ...
        loss = self.criterion(outputs, prediction_truth) * self.batch_size
    ...
    def val_batch(self, batch_scene, batch_scene_goal, batch_split):
        ...
        with torch.no_grad():
            ...
            rel_outputs_test, outputs_test = self.model(observed_test, batch_scene_goal, batch_split, n_predict=self.pred_length)
            # outputs_test = outputs_test[-self.pred_length:-1]
            ...
            prediction_truth = prediction_truth[:, primary_indicies]
            outputs = outputs[-self.pred_length + 1:, primary_indicies]
            outputs_test = outputs_test[-self.pred_length + 1:, primary_indicies]
            ...
            loss_test = self.criterion(outputs_test, prediction_truth) * self.batch_size
