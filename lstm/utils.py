...
def filter_opposite_x_velocity(xyv):
    """
    Removes neighbor cars whose xVelocity is in the opposite direction of the primary car (car 0).

    Args:
    - xyv (np.array): Array with shape [time, cars, data] where data has the form [x, y, xVelocity, yVelocity].

    Returns:
    - filtered_xyv (np.array): Array with cars that don't have opposing xVelocity to car 0.
    """
    # 获取0号车辆的xVelocity方向
    primary_car_x_velocity = xyv[:, 0, 2]

    # 根据主车的xVelocity方向创建一个布尔掩码，其中True表示对应的邻居车辆应该保留
    mask = np.sign(xyv[:, 1:, 2]) == np.sign(primary_car_x_velocity)[:, np.newaxis]

    # 使用布尔掩码过滤邻居车辆，仅保留那些与主车有相同xVelocity方向的邻居车辆
    mask_all = np.hstack([np.ones((mask.shape[0], 1), dtype=bool), mask])
    filtered_xyv = xyv[:, mask_all[0]]
