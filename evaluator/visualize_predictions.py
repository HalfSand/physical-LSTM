...
def main():
    ...
    parser.add_argument('--scale_factors', default=[2, 0.5, 0.01, 0.02],
                        help='give a scale factor which is the same as that in car.py')
    ...
    scaler = np.array(args.scale_factors)
    ...
    for scene_id, paths in scenes:
        ...
        #     for row in primary:
        #         # 解包tuple
        #         frame, car_id, x, y, *rest = row
        #         # 缩放x和y
        #         x *= scaler[0]
        #         y *= scaler[1]
        #         # 重新创建tuple并添加到新的列表中
        #         new_primary.append((x, y))
        #     pred_paths[name] = new_primary
